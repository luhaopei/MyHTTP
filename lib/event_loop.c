...
int event_loop_handle_pending_add(struct event_loop *eventLoop, int fd, struct channel *channel) {
    myhttp_msgx("add channel fd == %d, %s", fd, eventLoop->thread_name);
...
int event_loop_handle_pending_update(struct event_loop *eventLoop, int fd, struct channel *channel) {
    myhttp_msgx("update channel fd == %d, %s", fd, eventLoop->thread_name);
    struct channel_map *map = eventLoop->channelMap;
...
int channel_event_activate(struct event_loop *eventLoop, int fd, int revents) {
    struct channel_map *map = eventLoop->channelMap;
    myhttp_msgx("activate channel fd == %d, revents=%d, %s", fd, revents, eventLoop->thread_name);
...
int handleWakeup(void *data) {
    ...
    myhttp_msgx("wakeup, %s", eventLoop->thread_name);
}
...
#ifdef EPOLL_ENABLE
    myhttp_msgx("set epoll as dispatcher, %s", eventLoop->thread_name);
    eventLoop->eventDispatcher = &epoll_dispatcher;
#else
    myhttp_msgx("set poll as dispatcher, %s", eventLoop->thread_name);
...
int event_loop_run(struct event_loop *eventLoop) {
    ...
    myhttp_msgx("event loop run, %s", eventLoop->thread_name);
    ...
    myhttp_msgx("event loop end, %s", eventLoop->thread_name);
    return 0;
}
